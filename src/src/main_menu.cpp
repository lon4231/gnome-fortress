...
struct menu_data_t
{
    ...
    SDL_Rect play_button_trect;
    ui_button_t play_button;
};
...
void draw_title()
{
    ...
    for (uint8_t i = 0; i < 14; ++i)
    {
        ...
        SDL_Rect temp_rect = {
            (RENDER_TEXTURE_HW - 12 * 16) + (i * 32) + (int)(cosf(((i * 16) + SDL_GetTicks() * 0.1) * DEG_TO_RAD) * 4),
...
void play_menu_update()
{
    ...
    buttons_rect_ptr = &menu_data.play_button_trect;
    handle_ui_button(&menu_data.play_button);
    ...
    SDL_RenderCopy(window_hnd.renderer, runtime_assets.buttons_texture, &menu_data.play_button_trect, &menu_data.play_button.rect);
