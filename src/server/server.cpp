...
void init_server(uint16_t port)
{
    ...
    while (window_hnd.running && (clients_connected <= srv_state.client_count))
    {
        ...
        if (client_socket)
        {
            srv_state.clients[clients_connected].client_socket = client_socket;
            srv_state.clients[clients_connected].client_addr = *SDLNet_TCP_GetPeerAddress(srv_state.clients[clients_connected].client_socket);
            ;
